(function () {
    var ftpProfileController = function ($scope, $http, $timeout, $location, $state, $rootScope, $window, $cookies, usSpinnerService, Idle, Keepalive, $uibModal, manageUserProfileHttpService) {
        var data = [];
        $scope.profiles = []
        $scope.changeCount = 0;
    ...
    fccApp.controller("ftpProfileController", ["$scope", "$http", "$timeout", "$location", "$state", "$rootScope", "$window", "$cookies", "usSpinnerService", "Idle", "Keepalive", "$uibModal", "manageUserProfileHttpService", ftpProfileController]);
}())